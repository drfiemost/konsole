...
connect(_emulation, &Konsole::Emulation::titleChanged,
    ...
    connect(_emulation, &Konsole::Emulation::sessionAttributeRequest,
            this, &Konsole::Session::sessionAttributeRequest);
...
QString Session::getDynamicTitle()
{
    ...
    if (title.contains(QLatin1String("%D"))) {const QString homeDir = process->userHomeDir();
        if (!homeDir.isEmpty()) {
            QString tempDir = dir;
