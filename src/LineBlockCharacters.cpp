...
static inline uint lineWidth(uint fontWidth, bool heavy, bool bold)
{
    ...
    const uint  lightWidth          = qRound(std::max(baseWidth * boldCoeff, minWidth));
    const uint  heavyHalfExtraWidth = qRound(std::max(lightWidth * HeavyHalfExtraToLightRatio, 1.0));
                                           ...
                                           bool bold)
{
    ...
    const qreal halfGapH   = std::max(w / 20.0, 0.5);
    const qreal halfGapV   = std::max(h / 26.0, 0.5);
    // For some reason vertical double dash has bigger gap
    const qreal halfGapDDV = std::max(h / 14.0, 0.5);
